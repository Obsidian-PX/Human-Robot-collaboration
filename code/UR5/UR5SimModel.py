...
# UR5 simulation model in CoppeliaSim   CoppeliaSim中的UR5仿真模型
class UR5SimModel():
    ...
    def initializeSimModel(self, client_ID):
        ...
        # Get information from VREP for the first time  首次从VREP获取信息
        for i in range(6):
            return_code, q = vrep_sim.simxGetJointPosition(self.client_ID, self.joint_handle[i], vrep_sim.simx_opmode_streaming)

        # Set the initialized position for each joint  设置每个关节的初始化位置
        for i in range(6):
...
if __name__ == "__main__":
    ...
    # ------------------------------- Initialize simulation model ------------------------------- 初始化仿真模型
