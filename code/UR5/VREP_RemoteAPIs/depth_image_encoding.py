...
"""
#从浮点深度数据的numpy数组创建图像。图像转换首先对值进行缩放和舍入，然后在将数组转换为PIL图像对象之前，将它们打包为numpy数组中所需的类型。图像可以以任何格式保存。我们对RGB使用PNG，对灰度图像使用高质量JPEG。
#您可以使用不同的数字类型（例如np.uint16、np.int32），但PIL或标准图像查看器并不支持数字类型和图像格式的所有组合。
import numpy as np
