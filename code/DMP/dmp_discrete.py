...
class dmp_discrete():
    def __init__(self, n_dmps=1, n_bfs=100, dt=0, alpha_y=None, beta_y=None, **kwargs):
        ...
        self.n_bfs = n_bfs # number of basis functions  基函数数
        self.dt = dt
    ...
    def generate_weights(self, f_target):
        ...
        for d in range(self.n_dmps):
            ...
            for b in range(self.n_bfs):
                ...
                # numer = np.sum(psi_track[:,b] * f_target[:,d]) # the simpler calculation  更简单的计算
    ...
    def step(self, tau=None):
        ...
        for d in range(self.n_dmps):
            ...
            # ------------ 原始DMP Original DMP in Schaal 2002
            # f = np.dot(psi, self.w[d])*x*(self.goal[d] - self.y0[d]) / np.sum(psi)

            # ---------- Modified DMP in Schaal 2008, fixed the problem of g-y_0 -> 0  修改后的DMP，修正了g-y_0->0的问题
            # k = self.alpha_y[d]
            # f = k*(np.dot(psi, self.w[d])*x / np.sum(psi)) - k*(self.goal[d] - self.y0[d])*x # Modified DMP
            
            # ---------- Modified DMP with a simple solution to overcome the drawbacks of trajectory reproduction  用简单的解决方案改进DMP以克服轨迹再现的缺点
            k = self.alpha_y[d]

            self.delta_2[d] = self.goal[d] - self.y0[d] # Modified DMP extended  修改的DMP扩展
...
if __name__ == "__main__":
    ...
    # set new initial and goal poisitions  设定新的初始和目标位置
